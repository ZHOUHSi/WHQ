...
void Satlike::increase_hard_weights()
{
    int i, c, v;
...
void Satlike::increase_soft_weights()
{
    int i, c, v;
...
void Satlike::smooth_hard_weights()
{
    int i, clause, v;
...
void Satlike::smooth_soft_weights()
{
    int i, clause, v;
...
{
    // 更新硬子句权重   
    if (best_soln_feasible == 1 && ((double)(rand() % MY_RAND_MAX_INT) * BASIC_SCALE) < smooth_probability && large_weight_clauses_count > large_clause_count_threshold)
    {
        smooth_hard_weights();
    }
    else
    {
        increase_hard_weights();
    }

    // 更新软子句权重
    if (soft_unsat_weight >= opt_unsat_weight)
    {
        if (((double)(rand() % MY_RAND_MAX_INT) * BASIC_SCALE) < soft_smooth_probability && soft_large_weight_clauses_count > soft_large_clause_count_threshold)
        {
            smooth_soft_weights();
        }
        else if (hard_unsat_nb == 0)
        {
            increase_soft_weights();
        }
    }                
